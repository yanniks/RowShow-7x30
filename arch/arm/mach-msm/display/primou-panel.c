...
static uint32_t display_on_gpio_table[] = {
	LCM_GPIO_CFG(PRIMOU_LCD_PCLK_1, 1),
...
static uint32_t display_off_gpio_table[] = {
	LCM_GPIO_CFG(PRIMOU_LCD_PCLK_1, 0),
...
static int panel_sony_power(int on)
{
	...
	if (on) {
		hr_msleep(10);
		gpio_set_value(PRIMOU_LCD_RSTz_ID1, 1);
		hr_msleep(10);
		gpio_set_value(PRIMOU_LCD_RSTz_ID1, 0);
		udelay(500);
		gpio_set_value(PRIMOU_LCD_RSTz_ID1, 1);
		hr_msleep(10);
	} else if (!on) {
...
mddi_hitachi_power(u32 on)
{
  printk(KERN_ERR "%s: %d\n", __func__, on);
	if (panel_type == PANEL_ID_SAG_HITACHI) {
          ...
          gpio_set_value(PRIMOU_MDDI_RSTz,0);
          vreg_enable(vreg_ldo20);
          hr_msleep(1);
          gpio_set_value(PRIMOU_MDDI_RSTz,1);
          hr_msleep(5);
        }
        return 1;
}
